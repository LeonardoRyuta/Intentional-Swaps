...
/// Note: init attribute removed to avoid conflict with main canister init
pub fn init_bitcoin(network: Network) {
    init_upgrade(network);
}

/// Post-upgrade hook.
/// Reinitializes the BitcoinContext with the same logic as `init`.
/// Note: post_upgrade attribute removed to avoid conflict with main canister
pub fn upgrade_bitcoin(network: Network) {
    init_upgrade(network);
}
